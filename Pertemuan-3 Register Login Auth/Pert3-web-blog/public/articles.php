...
// Fitur search
// ---------------------------
// minta inputan string atau misal kita mau searching dia bakal auto ngebaca tulisan kecil
$searchQuery = isset($_GET['q']) ? strtolower(trim($_GET['q'])) : '';
// filter berdasarkan artikel
$filtered = $articles;

// kalau misal searching an kita itu enggak kosong
if ($searchQuery !== '') {
  $filtered = array_filter($articles, function ($a) use ($searchQuery) {
    // maka dia akan mengembalikan nilai judul dan deskripsi
...
// pagination config
$perPage = 6; // <-- tampilkan 3 artikel per halaman
$totalItems = count($filtered); // menghitung seluruh jumlah artikel
$totalPages = (int) max(1, ceil($totalItems / $perPage)); // ini dia ngebatesin artikel dengan pagination

// current page (sanitize)
$page = isset($_GET['page']) ? (int) $_GET['page'] : 1;
// jika suatu halaman atau artikel nampilin < 1 maka dia akan nampilin pagination
if ($page < 1) $page = 1;
// jika suatu artikel atau page itu lebih besar dari jumlah page nya maka dia tidak akan nampilin pagination lagi
...
<div class="mt-8 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
  <!-- kalau misal sebuah artikel itu lebih besar dari 0 -->
  <?php if (count($articlesPage) > 0): ?>
    <!-- maka dia akan menampilkan semua jumlah artikel yang kita miliki -->
    <?php foreach ($articlesPage as $article): ?>
      ...
      <!-- end buat nampilin semua data -->
    <?php endforeach; ?>
    <!-- kalau misal tidak ada artikel yang mau kita tampilin -->
  <?php else: ?>
    ...
    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
      <div>
        <p class="text-sm text-gray-700">
          Showing
          <!-- nampilin current page -->
          <span class="font-medium"><?= $start + 1 ?></span>
          to
          <!-- nampilin berapa jumlah artikel pada current page tersebut -->
          <span class="font-medium"><?= min($start + $perPage, $totalItems) ?></span>
          of
          <!-- nampilin semua total artikel -->
